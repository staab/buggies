// How a tire grips: the lateral force curve, the drive and retarding
// forces, the hold a tire at rest takes of the ground, and the friction
...
/**
 * Below this slip speed, with nothing asked of it, a tire takes hold of the
...
/**
 * A held tire pulls back to where it took hold like a spring, for each
...
export interface TireDriveContext {
  throttle: number
  brake: number
  handbrake: boolean
...
function driveForce(wheel: WheelState, drive: TireDriveContext, tuning: VehicleTuning): number {
...
function retardingForce(wheel: WheelState, drive: TireDriveContext, tuning: VehicleTuning, dt: number): number {
...
/**
 * Take hold of the ground if the tire is as good as still and nothing is
...
/** The pull of a held tire along one of its axes: back to where it took hold, and against its slip. */
function holdForce(displacement: number, slipSpeed: number, drive: TireDriveContext, dt: number): number {
...
function lateralGrip(wheel: WheelState, drive: TireDriveContext, tuning: VehicleTuning): number {
...
export function solveTireForces(wheel: WheelState, drive: TireDriveContext, tuning: VehicleTuning, dt: number): void {
  ...
  let lateral: number
  if (wheel.held) {
    // Held: pulled back to where it took hold, as hard as a tire can pull.
